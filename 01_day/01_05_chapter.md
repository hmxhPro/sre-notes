...
git操纵的是快照流，如果下一个项目版本只改动了部分文件，git只记录改动的文件，而不是整个项目，原有未改动的文件不会复制，会使用软连接指向原有文件
...
修改，创建新的分支，提交，合并，删除分支，合并分支冲突时，需要手动解决冲突

修改：
git add .
git commit -m "修改了文件"

第一次提交时需要添加远程仓库,git remote add origin <url> 然后再push
